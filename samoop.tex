...
a) èas MFR $\leq$ èas na vyhledání A + poèet placených výmìn A - poèet volných
výmìn A - $|P|$ 
  kdy¾ S1 = S2 \\
b) èas MFR $\leq$ èas na vyhledání A + poèet placených výmìn A - poèet volných
výmìn A - $|P|$ + $\binom{|S|}{2}$ 
  kdy¾ S1 $\neq$ S2
\end{theorem}
...
ii) x je v seznamu. Pøedpokládejme, ¾e x je na i-tém místì v seznamu $S_A$
...
DELETE(x)  bal($S_A''$, $S_{MFR}'$) - bal($S_A$, $S_{MFR}$) $\leq$ -k
...
% thanks to Jana Skotakova za zapisky
...
\begin{algorithmic}
\STATE Splay($\infty$, $T_2$) // nejmen¹í prvek\\
...
Amortizovaná slo¾itost této operace =
= èas operace + bal(nová konf.) - bal(pùvodní konf.) =
= 1 + r'(u) - r(v) - r(u) - r(t) (*)
...
r(u) \geq r(t), 1 \leq r(v) - r(t)
(*) \leq r(v) - r(t) + 2r(u) - 2r(t) = 3(r(v) - r(t))
$$

b2) 
$$
r(v) = r(t), pak podle lemmatu r'(t) > min{r'(u), r'(v)}
pak 2r'(t) \geq r'(u) + r'(v) + 1
(*) \leq 2r(u) - 2(r(t) = 2(r(v) - r(t)) = (r(t) = 0) 3(r(u) - 3r(t))
$$
\end{proof}
