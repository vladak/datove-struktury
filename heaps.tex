...
kdy¾ vrchol není list, pak ka¾dý vrchol s men¹ím èíslem má d synù.
\end{enumerate}

\item podmínku na lokální uspoøádání: \\
kdy¾ x je prvek pøiøazený vrcholu t, pak otci(t) je pøiøazen prvek
$\leq$ x
pak po oèíslování prùchodem do ¹íøky platí:
...
A: if prvek reprezentovaný x $>$ prvek reprezentovaný nìkterým synem x
...
prvek reprezentovaný posledním listem dáme do koøene, odstraníme poslední list \\
DOWN(koøen)

\subsubsection{DECREASEKEY$(x, \Delta)$}
musíme znát polohu vrcholu t reprezentujícího x, toto halda neumo¾òuje nalézt. \\
...
\begin{algorithm}[!htb]
\caption{Spojení dvou binom. hald}
...
\begin{pozn}
XXX
Sèítání v binárních èíslech má (XXX amort?) slo¾itost $O(1)$.
\\
1 0 0 ... 0 \\
1 1 1 ... 1 \\
\\
...
Zmìníme definici - vynecháme podmínku 2, tj. teï v na¹í binom. haldì
mohou být izomorfní stromy.
\par
...
DELETEMIN a MIN vy¾adují èas $O(\text{poèet stromù})$.
\end{theorem}
