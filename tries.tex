...
\begin{algorithmic}
t <- koøen
while t neni list do
  i <- hladina(t), t <- $(a_{i+1})$-ní syn t
done
if prvek(t) neni prefix x then
  beta = nejvetsi spolecny prefix x a prvek(t)
  beta a = prefix alfa
  beta b = prefix prvek(t)
  while hladina(t) > |beta| do t <- otec(t) done
  if hladina(t) < |beta| then
    vytvorime novy vrchol w, jehoz synove, krome b-teho syna budou listy s
    funkcemi Nal = false
    prvek(t) = beta + oznaceni syna
    hladina(w) = |beta|, beta = $(a_1, ..., a_i)$
    necht $v = a_{hladina(t) + 1}$ - tý syn t, b-ty syn w je v
    $w = a_{hladina(t)+1}$-tý syn t
  endif
  z <- a-ty syn t, Nal(z) = true, prvek(z) = x
else
...
t <- koren
while t neni list do 
  i <- hladina(t), t <- $(a_{i+1}$-ni syn t
done
if Nal(t) = true & prvek(t) = j then
  Nal(t) = false
  v <- otec(t)
  prvek(t) <- prefix prvek(t) o delce hladina v+1
  if vsichni synove vrchovlu v az na jednoho jsou listy s Nal = false then
    w <- syn(v), ktery je bud list s Nal(w) = true nebo neni list
    necht v je a-ty ($a_i$-ty ???) syn sveho otce, v smazeme a smazeme
    vsechny syny $v \neq w$
    w <- a-ty ($a_i$-ty ???) syn otce v
  endif
endif  
\end{algorithmic}
...
tj. \exist øádek i' pøedcházející a $\exist j,j'$ takové, ¾e $M_{i',j'} \neq
...
Mámme vektor v dimenze $n\cdotd$ a $i_0 < i_1 < ... < i_{+}$ jsou v¹echny
...
base(j) = -1 kdy¾ $i_k div d \neq j \forall k=0,1,...,t-1$
          $min{l; i_l div d = j}$ kdy¾ $\exist l, ¾e i_l div d = j$
