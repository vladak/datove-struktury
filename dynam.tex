...
\chapter{Dynamizace a semidynamizace}
...
\subsection{Algoritmus pro výpoèet}
...
\subsection{Èas pro INSERT(x)}
\begin{multline}
\underbrace{log|A|}{èas pro zji¹tìní zda x \in A} + \sum_{i=0}^{log|A|} (\frac{P(2^i)}{2^i} + 1) = 
...
\begin{algorithmic}
\IF {$x \not \in A$}
  \STATE nalezneme nejmen¹í j takové, ¾e $|\bigcup i\leqj A_i| < 2^j$
...
\mnote{tady nevím jestli nemá být jenom $A_{j-1}$}
    \ENDIF
