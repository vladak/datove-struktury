...
$H$ mu¾eme zajistit (nebo se k splnìní tohoto po¾adavku pøiblí¾it), ale 
...
\mnote{\scriptsize Markovova: $\pr(X \geq t \mathrm{E}X) \leq 1/t$}
\begin{theorem}
Za stejných pøedpokladù jako u pøedchozí vìty, kdy¾ $\mu$ je prùmìrná
...
a tedy
\[
\pr(|S_x| \geq t \mu) =  \frac {|H'|}{|H|} < \frac 1t
\]
\end{proof}
...
tedy \( \log _{2}|H|=2\log _{2}N \), tj. je hodnì velké 
...
$S \subseteq U$ perfektní ha¹ovací funkci, tj. funkci, která nemá na 
mno¾inì $S$ kolize. Tato úloha nepøipou¹tí pøirozenou implementaci 
operace INSERT, proto¾e pøidaný prvek mù¾e zpùsobit kolizi. 
Typický pøíklad pou¾ití je tabulka
klíèových slov kompilátoru.

\begin{defn}
Funkce $h: U \to \intrange{0}{m-1}$ je perfektní pro $S$, kdy¾ 
$\forall x\neq y \in S$ platí $h(x) \neq h(y)$.
\end{defn}
...
k(x-y) \bmod N & = r m && \text{pro } r \in 
	\{ - \lceil N/m \rceil .. \lceil N/m \rceil \} \minus \{0\}, \\
\end{align*}
tedy
\[
...
spor.
Pøi hledání $k^*$ ovìøíme perfeknost $h_k$ v èase $O(n)$.
%TODO! k jako sèítací index i optimální hodnota
\end{enumerate}
\end{proof}
...
prvoèíslo $p_0$ takové, ¾e $f_{p_0}(x) = x \bmod p_0$ je perfektní na
$S$ a $p_0 = O(n^2 \log N)$.
\end{lemma}

Vyu¾ití: pro $S$ najdeme prvoèíslo $p$ velikosti $O(n^2 \log N)$
takové, ¾e $f_p$ je perfektní na $S$. Vytvoøíme 
\[
S_0 = \{ f_p(s) : s \in S\} \subset \intrange{0}{p-1}
\]
a na $S_0$ aplikujme pøedchozí postup, kde $N = p$.
