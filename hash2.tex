...
% ..........................................................................
\subsection{Reprezentace a (MEMBER), INSERT, DELETE}
...
Spoèítání nové reprezentace:
\begin{enumerate}
\item loop
\item \quad zvolíme náhodnì $h \in H_i$ 
\item \quad spoèítáme reprezentaci $S$ vùèi $h$ 
\item until v¹echny øetìzce mají délku $\leq d_i$ 
\end{enumerate}

Kolikrát probìhne ten cyklus? Závisí to na více parametrech a Koubek
to nikde uspokojivì spoèítané nevidìl.

% --------------------------------------------------------------------------
\section{Externí ha¹ování}

Máme dáno universum U a vnìj¹íí pamì»ové medium, které je rozdìleno do
strnánek. Ka¾dá stránka mù¾e obsahovat nejvý¹e b záznamù. Chceme navrhnout
...
byly dostateènì zaplnìné.
\par
Vyhledáme v¾dy stránku ve vnìj¹í pamìti - natáhneme ji do vnitøní pamìti a
tam provedeme vyhledávání - toto vyhlledávání nás ale nezajímá.
\par
Nechh» $U$ je universum, $S \subseteq U$. Externí medium má stránky o
...
s \in Si$. Pro vrchol $\alpha$ jsou jeho synové $\alpha 0$ a $\alpha 1$
(pokud existují). Listy jsou prvky $h(s), s \in S$.

\begin{defn}
Pro $s \in S$ definujeme $\emph{d(s)} =$ délka nejmen¹ího prefixu $\alpha$ 
slova $h(s)$, ¾e pod $\alpha$ je nejvý¹e $b$ listù.
\end{defn}

XXX zde chybí obrázek s pøíkladem
0100, 0001, 0000, 1001
b = 2, d(S) = 2
0010 ... INSERT
...
pak $d(s) = d(t)$. $d(S) = max\{d(s), s \in S\}$.

Reprezentace: 
\begin{itemize}
\item adresáø: ke ka¾dému slovu $\alpha$ délky $d(S)$ je pøiøazena adresa stránky,
která obsahuje prvky $s \in S$, ¾e $h(s)$ má prefix $\alpha$
...
Operace ACCESS vy¾aduje 3 pøístupy na externí medium (pøedpoklad, ¾e
adresáø je také ulo¾en na externím mediu)
\par
Pro rychlou implementaci aktualizaèních operací je vhodné mít u ka¾dé
stránky ulo¾eno kolik je prvkù na stránce.
...
stránky odpovídající prefixu h(x) délky d(S).
\item Natáhneme do pamìti odpovídající stránku, kdy¾ existuje x - konec
\item Kdy¾ nneobsahuje x a má ménì prvkù ne¾ b, vlo¾íme x do této stránky
...
(nalezneme nové d(s) pro s z této stránky i s pøidaným x), stránky ulo¾íme
a aktualizujeme adresáø.
\end{enumerate}

v na¹em pøíkladu - zvìt¹í se adresáø.

00 - 0000, 0001		000 - 0000, 0001
01 - 0100		001 - 0010
10 \  			010 - 0100
    - 1001         =>   011 /
11 /			100 \
...
0010
d(S) = 1
% d(S \sjednoceno {0010}) = 3

\subsection{DELETE(x)}

\begin{enumerate}
\item spoèítáme h(x), natáhneme adresáø, nalezneme d(S), naleznneme adresu
...
stránky, který se spojí s na¹í stránkou
...
stránce)
\item kdy¾ obsahuje a dojde ke sluèování, ppak odstraníme x, natáhneme
druhou stránku a stránky slouèíme, ullo¾íme novou stránku a aktualizujeme
adresáø.
\end{enumerate}
...
\begin{pozn}
Pesimistický odhad: nejvý¹e 6 pøístupù na externí medium.
\end{pozn}
...
adresáè vymazáním sudých èlenù posloupnosti.
\end{itemize}
...
\begin{pozn}
èlen $n^{1 + \frac{1}{b}}$ není lineární, zde je problé, proto se to
nehodí pro malá $b$.
\end{pozn}
...
$b \geq 100$, pro vìt¹í mno¾iny S, b musí být vìt¹í..
\end{pozn}
...
% *ceil -> *floor (fakticka poznamka od Ladislava Proska
	\{ - \lfloor N/m \rfloor .. \lfloor N/m \rfloor \} \minus \{0\}, \\
\end{align*}
tedy
\[
...
\mnote{poøádnou bibliografii}
Èlánek se dá stáhnout z \href
...
\mnote{Tohle u¾ Koubek nepøedná¹í}
Adresáø.
