...
\WHILE {$t$ \text{není list} \text{ a } $t$ \text{nereprezentuje} $x$} 
...
Algoritmus pro výpoèet $H$ a $K$ jsme modifikovali, tak ¾e øádky 
(1),(2),(3) jsme nahradili øádky (1'),(2'),(3').
...
$p(v)$ je délka nejdel¹í cesty z $v$ do listu v podstromu pravého syna
$v$. Pokud takový podstrom neexistuje, polo¾me $l(v)$ resp. $p(v)$ rovno $-1$.
Dále oznaème $b(v) = l(v) - p(v)$. Vrchol $v$ nazveme vyvá¾ený, jestli¾e
$b(v)$ nebývá hodnot $-1$, $0$ nebo $1$.
\end{defn}
...
\item $N(0) = 1 \geq 2^{0/2} = 1$ \\
   $N(1) = 2 \geq 2^{1/2}$
\item $N(h) \geq 2 N(h-2) \geq 2 2^{h/(2-1)} = 2^{h/2}$
\end{enumerate}
...
x nevyvá¾ený. Jeho vyvá¾ení se v¹ak provede jednodu¹e pomocí tzv.
LL-rotace. (viz. obr. \ref{avl-ll})
...
vyvá¾it. Samozøejmì, ¾e tento vrchol $x$ le¾í na cestì od koøene k pøidanému
vrcholu a platí:
\begin{itemize}
...
podstromu. Pøi tom se samozøejmì mohou také nìkteré vrcholy stát
