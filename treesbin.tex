...
$\alpha_1, ..., \alpha_n, \beta_1, ..., \beta_n$, kde 
\begin{itemize}
...
\begin{pozorov}
% Pozorování 2: 
Nech» platí pøedpoklady Pozorování 1 a nechh» T je optimální
...
Kdy¾ $hod(T_{i,k-1}) + hod(T_{k+1,j})
\leq min\{hod(T_{i,k'-1}) + hod(T_{k'+1,j}), i \leq k' \leq j\}$, pak \\
$\exists$ opt. bin. vyhl. strom pro $S_{i,j}$ jeho¾ koøen reprezentuje $x_k$.
\end{pozorov}
...
\begin{defn}
Binární vyhledávací strom T se nazývá èervenoèerný, jestli¾e ka¾dý 
...
platí, ¾e jejich hloubka je $O(log n)$.
\end{theorem}
